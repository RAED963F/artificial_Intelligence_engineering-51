...
def compute_quality_flags(
    ...
    # НОВЫЕ ЭВРИСТИКИ (требуется DataFrame)
    if df is not None:
        # 1. Постоянные столбцы
        constant_columns = []
        ...
        if len(constant_columns) > 0:
            score -= 0.15
            if verbose:
                typer.echo(f"⚠️  Обнаружены постоянные столбцы: {len(constant_columns)} шт.")
        
        # 2. Выбросы в числовых столбцах
        outlier_columns = []
        ...
        flags["has_outliers"] = len(outlier_columns) > 0
        flags["outlier_columns"] = outlier_columns
        flags["n_outlier_columns"] = len(outlier_columns)
        
        if len(outlier_columns) > 0:
            penalty = min(0.3, len(outlier_columns) * 0.05)
            score -= penalty
            if verbose:
                typer.echo(f"⚠️  Обнаружены выбросы в {len(outlier_columns)} числовых столбцах")
